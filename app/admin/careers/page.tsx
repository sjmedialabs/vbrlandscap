...
const perkIcons = ["Sprout", "TreeDeciduous", "Users", "Leaf", "Heart", "Star", "Briefcase", "Award"]
const cultureIcons = ["Settings", "Brain", "Users", "BookOpen", "Wrench", "Leaf", "Cog", "Lightbulb", "Target", "Zap"]
...
export default function AdminCareersPage() {
  ...
  const tabFromUrl = searchParams.get("tab") as "hero" | "perks" | "culture" | "environment" | "cta" | null
  const [activeTab, setActiveTab] = useState<"hero" | "perks" | "culture" | "environment" | "cta">(tabFromUrl || "hero")

  const [pageData, setPageData] = useState<CareersPageData>(defaultCareersPageData)
  const [saving, setSaving] = useState(false)
  const [saveStatus, setSaveStatus] = useState<"idle" | "success" | "error">("idle")

  // Sync tab with URL changes
  useEffect(() => {
    const tab = searchParams.get("tab") as "hero" | "perks" | "culture" | "environment" | "cta" | null
    if (tab && ["hero", "perks", "culture", "environment", "cta"].includes(tab)) {
      setActiveTab(tab)
    }
  }, [searchParams])
  ...
  const updatePerk = (index: number, field: keyof PerkItem, value: string | number) => {
  ...
  const removeHighlight = (index: number) => {
    setPageData({
      ...pageData,
      cultureHighlights: pageData.cultureHighlights.filter((_, i) => i !== index),
    })
  }

  const updateHighlight = (index: number, field: keyof CultureHighlight, value: string | number) => {
    ...
    <div className="flex h-screen bg-[#f5f7f0]">
      ...
      <main className="flex-1 overflow-y-auto px-8 py-6">
        ...
        <div className="mb-6 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-[#1a1a1a]">Careers Manager</h1>
        ...
        {activeTab === "hero" && (
          <div className="rounded-xl border border-[#e5e5e5] bg-white p-6">
            <h2 className="text-lg font-semibold text-[#1a1a1a] mb-6">Hero Section</h2>
            <div className="space-y-6">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setPageData({ ...pageData, heroBreadcrumb: e.target.value })}
                  className="w-full rounded-lg border border-[#ddd] bg-white px-3 py-2 text-sm outline-none focus:border-[#2d6a2e]"
                />
              </div>
            </div>
        ...
        {activeTab === "perks" && (
          <div className="rounded-xl border border-[#e5e5e5] bg-white p-6">
            <div className="mb-6 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-[#1a1a1a]">Perks & Benefits</h2>
              <button
                onClick={addPerk}
            ...
            <div className="grid gap-4 sm:grid-cols-2 mb-6 pb-6 border-b border-[#e5e5e5]">
              <div>
                <label className="mb-1.5 block text-xs font-medium text-[#666]">Section Label</label>
                <input
                  type="text"
                  value={pageData.perksLabel}
                  onChange={(e) => setPageData({ ...pageData, perksLabel: e.target.value })}
                  className="w-full rounded-lg border border-[#ddd] bg-white px-3 py-2 text-sm outline-none focus:border-[#2d6a2e]"
                />
              </div>
              <div>
                <label className="mb-1.5 block text-xs font-medium text-[#666]">Section Heading</label>
                <input
                  type="text"
                  value={pageData.perksHeading}
                  onChange={(e) => setPageData({ ...pageData, perksHeading: e.target.value })}
                  className="w-full rounded-lg border border-[#ddd] bg-white px-3 py-2 text-sm outline-none focus:border-[#2d6a2e]"
                />
              </div>
            </div>
            ...
            <div className="space-y-4">
              {pageData.perks.map((perk, index) => (
                <div key={perk.id} className="rounded-lg border border-[#e5e5e5] bg-[#fafafa] p-4">
                  ...
                  <div className="grid gap-3 sm:grid-cols-3">
                    ...
                    <input
                      ...
                      placeholder="Order"
                      value={perk.order}
                      onChange={(e) => updatePerk(index, "order", parseInt(e.target.value) || 0)}
                      className="rounded-lg border border-[#ddd] bg-white px-3 py-2 text-sm outline-none focus:border-[#2d6a2e]"
                    />
                  </div>
                  <textarea
        ...
        {activeTab === "culture" && (
          <div className="rounded-xl border border-[#e5e5e5] bg-white p-6">
            <div className="mb-6 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-[#1a1a1a]">Culture Highlights</h2>
              <button
            ...
            <div className="space-y-4 mb-6 pb-6 border-b border-[#e5e5e5]">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setPageData({ ...pageData, cultureHeading: e.target.value })}
                  className="w-full rounded-lg border border-[#ddd] bg-white px-3 py-2 text-sm outline-none focus:border-[#2d6a2e]"
                />
              </div>
              <div>
                <label className="mb-1.5 block text-xs font-medium text-[#666]">Section Description</label>
                <textarea
                  value={pageData.cultureDescription}
                  onChange={(e) => setPageData({ ...pageData, cultureDescription: e.target.value })}
                  rows={2}
                  className="w-full rounded-lg border border-[#ddd] bg-white px-3 py-2 text-sm outline-none focus:border-[#2d6a2e]"
                />
              </div>
            ...
            <div className="space-y-4">
              {pageData.cultureHighlights.map((highlight, index) => (
                <div key={highlight.id} className="rounded-lg border border-[#e5e5e5] bg-[#fafafa] p-4">
                  <div className="mb-3 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <GripVertical className="h-4 w-4 text-[#999]" />
                      <span className="text-xs font-semibold uppercase text-[#888]">
                  ...
                  <div className="grid gap-3 sm:grid-cols-3">
                    ...
                    <input
                      ...
                      onChange={(e) => updateHighlight(index, "order", parseInt(e.target.value) || 0)}
                      className="rounded-lg border border-[#ddd] bg-white px-3 py-2 text-sm outline-none focus:border-[#2d6a2e]"
                    />
                  </div>
                  <textarea
        ...
        {activeTab === "environment" && (
          <div className="rounded-xl border border-[#e5e5e5] bg-white p-6">
            <h2 className="text-lg font-semibold text-[#1a1a1a] mb-6">Work Environment Section</h2>
            <div className="space-y-6">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setPageData({
                      ...pageData,
                      workEnvironment: { ...pageData.workEnvironment, heading: e.target.value },
                    })
                  }
                  className="w-full rounded-lg border border-[#ddd] bg-white px-3 py-2 text-sm outline-none focus:border-[#2d6a2e]"
                />
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setPageData({
                      ...pageData,
                      workEnvironment: { ...pageData.workEnvironment, description: e.target.value },
                    })
                  }
                  rows={4}
                  className="w-full rounded-lg border border-[#ddd] bg-white px-3 py-2 text-sm outline-none focus:border-[#2d6a2e]"
              ...
              <ImageUpload
                ...
                onChange={(url) =>
                  setPageData({
                    ...pageData,
                    workEnvironment: { ...pageData.workEnvironment, image: url },
                  })
                }
              />
            </div>
        ...
        {activeTab === "cta" && (
          <div className="rounded-xl border border-[#e5e5e5] bg-white p-6">
            <h2 className="text-lg font-semibold text-[#1a1a1a] mb-6">CTA Section</h2>
            <div className="space-y-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setPageData({
                        ...pageData,
                        ctaSection: { ...pageData.ctaSection, label: e.target.value },
                      })
                    }
                    className="w-full rounded-lg border border-[#ddd] bg-white px-3 py-2 text-sm outline-none focus:border-[#2d6a2e]"
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setPageData({
                        ...pageData,
                        ctaSection: { ...pageData.ctaSection, buttonText: e.target.value },
                      })
                    }
                    className="w-full rounded-lg border border-[#ddd] bg-white px-3 py-2 text-sm outline-none focus:border-[#2d6a2e]"
                  />
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setPageData({
                      ...pageData,
                      ctaSection: { ...pageData.ctaSection, heading: e.target.value },
                    })
                  }
                  className="w-full rounded-lg border border-[#ddd] bg-white px-3 py-2 text-sm outline-none focus:border-[#2d6a2e]"
                />
              ...
              <div>
                <label className="mb-1.5 block text-xs font-medium text-[#666]">Subheading</label>
                <input
                  type="text"
                  ...
                  onChange={(e) =>
                    setPageData({
                      ...pageData,
                      ctaSection: { ...pageData.ctaSection, subheading: e.target.value },
                    })
                  }
                  className="w-full rounded-lg border border-[#ddd] bg-white px-3 py-2 text-sm outline-none focus:border-[#2d6a2e]"
                />
              ...
              <div>
                <label className="mb-1.5 block text-xs font-medium text-[#666]">Email Placeholder</label>
                <input
                  type="text"
                  ...
                  onChange={(e) =>
                    setPageData({
                      ...pageData,
                      ctaSection: { ...pageData.ctaSection, placeholder: e.target.value },
                    })
                  }
                  className="w-full rounded-lg border border-[#ddd] bg-white px-3 py-2 text-sm outline-none focus:border-[#2d6a2e]"
                />
