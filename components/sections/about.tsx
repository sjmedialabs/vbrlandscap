  ...
  return (
    <section className="relative overflow-hidden bg-background py-20 lg:py-28">
      ...
      <div className="pointer-events-none absolute -top-8 -left-8 w-48 lg:w-72">
        <Image src={leafDecoration} alt="" width={300} height={400} className="h-auto w-full opacity-50" aria-hidden="true" />
      </div>
      <div className="pointer-events-none absolute -right-10 -bottom-6 hidden w-56 lg:block lg:w-72">
        <Image src={gardenToolsDecor} alt="" width={300} height={300} className="h-auto w-full" aria-hidden="true" />
      </div>
      ...
      <div className="relative mx-auto max-w-7xl px-6">
        <div className="flex flex-col gap-12 lg:flex-row lg:gap-16">
          ...
          <div className="relative w-full lg:w-[45%]">
            <div className="relative ml-6 mt-8 lg:ml-12 lg:mt-12">
              <div className="relative aspect-[3/4] w-full max-w-[480px] overflow-hidden rounded-2xl">
                <Image src={personImage} alt="Professional landscaper" fill sizes="(max-width: 1024px) 100vw, 45vw" className="object-cover" />
              </div>
            </div>
          </div>
          ...
          <div className="flex w-full flex-col justify-center lg:w-[55%]">
            ...
            <h2 className="mt-6 text-balance text-3xl font-bold leading-[1.15] text-foreground md:text-4xl lg:text-[46px]">{heading}</h2>
              ...
              <div className="flex flex-1 flex-col gap-6">
                <div className="flex items-start gap-4">
                  <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-muted">
                    <Image src={missionIcon} alt="" width={44} height={44} className="h-11 w-11 rounded-full" aria-hidden="true" />
                  </div>
                  <div>
                    <h3 className="text-[15px] font-bold text-foreground">{missionTitle}</h3>
                    <p className="mt-1 text-sm leading-relaxed text-muted-foreground">{missionDescription}</p>
                  </div>
                </div>
                <div className="flex items-start gap-4">
                  <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-muted">
                    <Image src={visionIcon} alt="" width={44} height={44} className="h-11 w-11 rounded-full" aria-hidden="true" />
                  </div>
                  <div>
                    <h3 className="text-[15px] font-bold text-foreground">{visionTitle}</h3>
                    <p className="mt-1 text-sm leading-relaxed text-muted-foreground">{visionDescription}</p>
                  </div>
                </div>
              </div>
              <div className="relative hidden h-52 w-52 shrink-0 overflow-hidden rounded-2xl lg:block">
                <Image src={containerImage} alt="Gardening tools" fill sizes="208px" className="object-cover" />
            ...
            <div className="mt-8 flex flex-col items-start gap-5 sm:flex-row sm:items-center sm:gap-6">
              <button type="button" className="group inline-flex items-center gap-3 rounded-full bg-[#c5d846] px-6 py-3 text-sm font-semibold text-foreground transition-all hover:brightness-95">
                {ctaButtonText}
