...
export default function ServicesSection({ data }: ServicesProps) {
  ...
  const items = ((data.items as ServiceItem[]) || []).sort((a, b) => (a.order ?? 0) - (b.order ?? 0))
    ...
    <section className="relative overflow-hidden bg-[#f5f3eb] py-20 lg:py-28">
      <div className="mx-auto max-w-7xl px-6">
        ...
        <div className="mt-16 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {items.map((service) => (
            <div key={service.title} className="group rounded-2xl bg-background p-5 pb-8 transition-all hover:-translate-y-1 hover:shadow-lg">
              <div className="flex items-center justify-center">
                <div className="relative h-[220px] w-[220px]">
                  <Image src={service.image} alt={service.title} fill sizes="220px" className="object-contain" />
                </div>
              </div>
        ...
        <div className="mt-14 flex flex-wrap items-center justify-center gap-4">
          <span className="rounded-full bg-[#c5c43e] px-6 py-2 text-sm font-bold text-foreground uppercase">{bottomTag}</span>
          <p className="text-sm text-muted-foreground md:text-base">{bottomText}</p>
          <a href="#" className="text-sm font-semibold text-foreground underline underline-offset-4 md:text-base">{bottomLink}</a>
        </div>
      </div>

      <div className="pointer-events-none absolute -bottom-4 right-0 h-[200px] w-[200px] lg:h-[280px] lg:w-[280px]">
        <Image src={leafImage} alt="" fill sizes="280px" className="object-contain" />
      </div>
    </section>
  )
}
